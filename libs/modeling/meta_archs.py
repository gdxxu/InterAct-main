...
from .heads import ActionHead, STI
...
class SP_STI(nn.Module):
    def __init__(self,
        ...
        self.node = 19 if dataset == 'LARA' else 25
        self.in_channel = in_channel // self.node

        self.SP = MultiScale_GraphConv(13, self.in_channel, n_features, dataset)

        self.num_modality = 3 if dataset == 'LARA' else 2
        self.num_person = 2 if dataset in ['PKU-subject', 'PKU-view', 'PKU-MMDv1'] else 1 
        self.in_channel //= self.num_modality
        self.SP_layers = nn.ModuleList(     
            [MultiScale_GraphConv(13, self.in_channel, n_features, dataset) for _ in range(self.num_modality)]
        )
        self.SI_layers = nn.ModuleList(     
            [SpatialInteraction(2*n_features, n_features) for _ in range(self.num_modality - 1)]
        )

    def forward(self, x, mask):
        B, _, T = x.size()
        x = x.view(B, self.num_person, self.num_modality, -1, self.node, T)
        x = x.permute(0, 2, 1, 3, 4, 5).contiguous().view(B, self.num_modality, -1, self.node, T)

        multi_scale_feat = self.SP_layers[0](x[:, 0, :, :, :]) * mask.unsqueeze(3) 
        for i in range(1, self.num_modality):
            feature = self.SP_layers[i](x[:, i, :, :, :]) * mask.unsqueeze(3)
            multi_scale_feat = self.SI_layers[i-1](multi_scale_feat, feature, mask)

        feature = multi_scale_feat * mask.unsqueeze(3)
        feature = feature.permute(0, 1, 3, 2).reshape(B, -1, T) 

        return feature
    
class SpatialInteraction(nn.Module):
    def __init__(self, n_features, out_channel) -> None:
        super().__init__()
        self.softmax = nn.Softmax(dim=-1)
        self.ff = nn.Sequential(nn.Linear(n_features, out_channel),
                                nn.ReLU(),
                                nn.Dropout(0.3),
                                nn.Linear(out_channel, out_channel))
    
    def forward(self, multi_feature, feature, mask):
        B, C, T, V = multi_feature.size()
        feature_qkv = torch.cat([multi_feature, feature], dim=1)  

        multi_feature = multi_feature.permute(0, 2, 1, 3).reshape(B*T, C, V)
        feature_qkv = feature_qkv.permute(0, 2, 1, 3).reshape(B*T, 2*C, V)
        map = self.softmax(torch.einsum("ncv, ndv -> ncd", feature_qkv, feature_qkv) / V) 
        feature_cross = torch.einsum("ncd, ndv -> ncv", map, feature_qkv) 
        feature_cross = feature_cross + feature_qkv
        feature_cross = feature_cross.permute(0, 2, 1)  
        feature_cross = self.ff(feature_cross).permute(0, 2, 1) + multi_feature 
        feature_cross = feature_cross.reshape(B, T, C, V).permute(0, 2, 1, 3) 

        return feature_cross * mask.unsqueeze(3)
    ...
    def __init__(
        ...
        if backbone_type == 'convTransformer':
            self.backbone = make_backbone(
                'convTransformer',
                **{
                    'n_in' : 1216 if dataset == 'LARA' else 1600,  # LARA: 64*(V=19)=1216; others: 64*(V=25)=1600
                    'n_embd' : embd_dim,
                    'n_head': n_head,
                    'conv_dila': conv_dila,
...
class MaskPtTransformer(PtTransformer):
    def __init__(
        ...
        # SP+STI: self.dataset is set by PtTransformer.__init__ via the dataset parameter
        self.SP_STI = SP_STI(input_dim, 64, embd_dim, 10, SFI_layer=[1,2,3,4,5,6,7,8,9], dataset=self.dataset)
    ...
    def forward(self, video_list):    
        ...
        # SP+STI
        batched_inputs = self.SP_STI(batched_inputs, batched_masks)
    ...
    def inference(self, multi_level_labels, action_mask_output, output_boundary, frame_cls_logits=False):
        ...
        for i in range(len(multi_level_labels)):
            ...
            if self.with_action:    # True
                ...
                pred = torch.argmax(action_mask_output[i].transpose(0,1)[:duration], -1)    # [T]
    ...
    def preprocessing(self, video_list, num_level, padding_val=0.0):
        ...
        if self.training:
            ...
            batched_labels = torch.ones(len(video_list), max_len) * self.ignore_index   # [B, T]
            ...
            # batch input shape B, C, T
            batch_shape = [len(feats), feats[0].shape[0], max_len]  # [B, input_dim, max_seq_len]
            batched_inputs = feats[0].new_full(batch_shape, padding_val)
            for feat, pad_feat in zip(feats, batched_inputs):
                pad_feat[..., :feat.shape[-1]].copy_(feat)
        else:
    ...
    def get_boundary(self, boundary_output, duration, idx):
        boundary_pred = boundary_output[idx].sigmoid()[:duration]   # [T]
        local_max = F.max_pool1d(boundary_pred.unsqueeze(0), 15, stride=1, padding=7)   
        local_max[local_max<self.boundary_thr] = 0  # self.boundary_thr = 0.1
