...
class Criterion(nn.Module):
    def __init__(self,
                    num_classes,
                    frame_cls_loss= False,          # False
                    frame_mask_loss= False,         # False
                    action_cls_loss= False,         # True
                    action_mask_loss= False,        # True
                    frame_action_cls_loss= False,
                    tmse_loss=False,                # False
                    boundary_loss=False,            # True

                    action_cls_cross_loss= False,   # False
                    action_cls_smooth_loss=False,   # False
    ...
    def loss_frame_mask(self, out_cls, gt_cls, b_mask):
        ...
        # [num_classes, T]; [T]; [T]
        gt_cls_onehot = F.one_hot(gt_cls+1, num_classes=self.num_classes+1) # [T, num_classes+1]
        gt_cls_onehot = gt_cls_onehot[:,1:] # [T, num_classes]

        # get actions
        action_unique = torch.unique(gt_cls)    
        action_unique = action_unique[action_unique!= -1]   
        action_gt_cls_onehot = gt_cls_onehot[:,action_unique].float()   # [T, num_classes] -> [T, num_actions]
        ...
        action_out_cls = out_cls[action_unique] # [num_classes, T] -> [num_actions, T]
        ### default dice loss   [num_actions, T]; [num_actions, T]; [T]
    ...
    def loss_action_cls(self, out_cls, gt_cls, mask):
        gt_cls_onehot = F.one_hot(gt_cls+1, num_classes=self.num_classes+1) # [B, T, num_classes+1]
        gt_cls_onehot = gt_cls_onehot[:, :,1:]      # [B, T, num_classes]
        gt_cls_onehot = gt_cls_onehot.flatten(0,1)  # [B*T, num_classes]
        out_cls = out_cls.transpose(-2, -1).flatten(0,1)    # [B*T, num_classes]
        mask = mask.flatten()   # [B*T]
    ...
    def loss_boundary(self, bb_out, bgt_f, bmask):  # [T]; [T]; [T]
        boundary_points = torch.arange(-self.boundary_len, self.boundary_len+1, device=bb_out.device)   # [2*boundary_len+1]
        gaussian_boundary = torch.exp(-0.5 * (boundary_points **2)/ (self.boundary_sigma **2))  # [2*boundary_len+1]
        ...
        gt_boundary = torch.zeros(len(bgt_f)).to(bb_out.device)             # [T]
    ...
    def forward(self, 
        ...
        if with_aux_outputs:    
            ...
            for la_out_mask in  a_out_mask:
                # action class loss
                gt_f_cls_select4cls = gt_f_cls[0] if type(gt_f_cls) is tuple else gt_f_cls  # [B, T] 
                mask_select4cls = masks[0] if type(masks) is list else masks                # [B, T]    
                if self.action_cls_loss:            # True  [B, num_classes, T]; [B, T]; [B, T] 
                    losses['action_cls_loss'] += self.loss_action_cls(la_out_mask, gt_f_cls_select4cls, mask_select4cls)/num_action_lvl # focal loss
                if self.action_cls_cross_loss:      # False
                ...
                if self.action_cls_smooth_loss:     # False
                    losses['action_cls_smooth_loss'] += self.loss_action_cls_smooth(la_out_mask, mask_select4cls)/num_action_lvl # smooth loss
                # action mask loss                  # True
                if self.action_mask_loss:
                    for b in range(bs):
                        a_out_mask_select4mask = la_out_mask    # [B, num_classes, T]
                        gt_f_cls_select4mask = gt_f_cls[0] if type(gt_f_cls) is tuple else gt_f_cls # [B, T] 
                        mask_select4mask = masks[0] if type(masks) is list else masks               # [B, T] 
                        ba_out_mask_select4mask = a_out_mask_select4mask[b] # [num_classes, T]
                        bgt_f_cls_select4mask = gt_f_cls_select4mask[b]     # [T]
                        bmask_select4mask = mask_select4mask[b]             # [T]
        ...
        if self.frame_action_cls_loss:      # False
        ...
        if self.boundary_loss:              # True
            if with_aux_boundary:
                ...
                for lb_out in b_out:        # (n_blocks+1)* [B, T]
                    for b in range(bs):
                        blb_out = lb_out[b] # [T]
                        mask_select4boundary = masks[0] if type(masks) is list else masks   # [B, T]
                        bmask_select4boundary = mask_select4boundary[b]         # [T]
                        gt_f_cls_select4boundary = gt_f_cls[0] if type(gt_f_cls) is tuple else gt_f_cls # [B, T]
                        bgt_f_cls_select4boundary = gt_f_cls_select4boundary[b] # [T]
