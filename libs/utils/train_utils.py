...
def make_optimizer(model, optimizer_config):
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            elif pn.endswith('mlp.layers.1.weight') or pn.endswith('mlp.layers.0.weight') or pn.endswith('conv_in.weight') or pn.endswith('A_res')\
                or pn.endswith('residual.0.weight') or pn.endswith('residual.1.weight') or pn.endswith('residual.3.weight') or pn.endswith('act.weight'):
                decay.add(fpn)
...
def train_one_epoch(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        # flops, params = profile(model.module, (data,))
        # print('flops: ', flops, 'params: ', params)
        # print('flops: %.2f G, params: %.2f M' % (flops / 1000000000.0, params / 1000000.0))
        losses = model(video_list)  # class_weights
