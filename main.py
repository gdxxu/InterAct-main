...
def main(args):
    ...
    # model
    cfg['model']['dataset'] = cfg['dataset_name']
    ...
    for epoch in range(args.start_epoch, max_epochs):
        ...
        train_one_epoch(
            ...
            epoch,
            model_ema = model_ema,
            clip_grad_l2norm = cfg['train_cfg']['clip_grad_l2norm'],    # 1
            tb_writer=tb_writer,
        ...
        if (
            ...
            total_f1 = eval_results['F1_0.1'] + eval_results['F1_0.25'] + eval_results['F1_0.5']
            if total_f1 > max_total_f1:
                max_total_f1_epoch = epoch + 1
                max_total_f1 = max(max_total_f1, total_f1)
                log_content = f"max total f1 epoch{epoch+1}"
                print(log_content)
                with open(log_file, 'a') as fid:
                    fid.write(log_content + '\n')
                    fid.flush()
                ...
                save_checkpoint(
                    ...
                    False,
                    file_folder=ckpt_folder,
                    file_name=f'max_total_f1.pth.tar'
                )
            
            mean = (eval_results['Acc'] + eval_results['Edit'] + total_f1) / 5
            if mean > max_mean:
                max_mean_epoch = epoch + 1
                max_mean = max(max_mean, mean)
                log_content = f"max mean epoch{epoch+1}"
                print(log_content)
                with open(log_file, 'a') as fid:
                    fid.write(log_content + '\n')
                    fid.flush()
                ...
                save_checkpoint(
                    ...
                    False,
                    file_folder=ckpt_folder,
                    file_name=f'max_mean.pth.tar'
                )

    log_content = f"max acc epoch{max_acc_epoch}; max edit epoch{max_edit_epoch}; max total f1 epoch{max_total_f1_epoch}; max mean epoch{max_mean_epoch} \n"
    print(log_content)
    with open(log_file, 'a') as fid:   
        fid.write(log_content + '\n')
        fid.flush()
